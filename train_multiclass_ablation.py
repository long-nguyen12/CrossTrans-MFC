...
def run_single_ablation(
    ...
    model = AblationModel(cfg, ablation_cfg, num_fine_per_coarse=NUM_FINE_PER_COARSE).to(device)
...
def main(args):
    ...
    print(f"{'═' * 90}")
    print(
        f"{'Variant':<8} {'Description':<45} {'Val FineF1':>10} {'Test CoarseF1':>13} {'Test FineF1':>11} {'Test FineAcc':>12}"
    )
    print(f"{'─' * 8} {'─' * 45} {'─' * 10} {'─' * 13} {'─' * 11} {'─' * 12}")
    for r in all_results:
        print(
            f"{r['variant']:<8} {r['description']:<45} "
            f"{r['best_val_fine_f1']:>10.4f} {r['test_coarse_f1']:>13.4f} "
            f"{r['test_fine_f1']:>11.4f} {r['test_fine_acc']:>12.4f}"
        )
    print(f"{'═' * 90}\n")
    ...
    if results_dir is not None:
        ...
        with open(results_dir / "ablation_summary.md", "w") as f:
            f.write("# Ablation Study Results (Hierarchical Multiclass)\n\n")
            f.write(
                "| Variant | Description | Params (trainable) | Val Fine F1 | Test Coarse F1 | Test Fine F1 | Test Fine Acc |\n"
            )
            f.write(
                "|---------|-------------|--------------------|-------------|----------------|--------------|---------------|\n"
            )
            for r in all_results:
                f.write(
                    ...
                    f"{r['best_val_fine_f1']:.4f} | {r['test_coarse_f1']:.4f} | "
                    f"{r['test_fine_f1']:.4f} | {r['test_fine_acc']:.4f} |\n"
                )
